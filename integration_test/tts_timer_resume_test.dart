  ...
  group('TTS Timer Resume - Real User Behavior Tests', () {
    ...
    group('CRITICAL: Pause/Resume Maintains Timer Position', () {
      test('User pauses at 10s, resumes - timer continues from 10s', () async {
        ...
        expect(pausedPosition.inSeconds, greaterThanOrEqualTo(0),
      ...
      test('Multiple pause/resume cycles maintain accumulated position',
          () async {
        // GIVEN: User listening to long devotional
        controller
            .setText('Texto largo para múltiples pausas y reanudaciones.');
        await controller.play();
        await Future.delayed(const Duration(milliseconds: 500));
        ...
        for (int i = 0; i < 3; i++) {
          ...
          expect(pausedPosition.inMilliseconds,
          ...
          expect(controller.currentPosition.value.inMilliseconds,
    ...
    group('CRITICAL: Speed Change Maintains Timer Position', () {
      test('User pauses, changes speed, resumes - timer continues correctly',
          () async {
    ...
    group('CRITICAL: Seek Updates Timer Correctly', () {
      test('User seeks to 30s - timer updates and continues from 30s',
          () async {
        // GIVEN: User is playing devotional
      ...
      test('Seek while paused, then resume - timer starts from seek position',
          () async {
    ...
    group('CRITICAL: Modal Close/Reopen Maintains Position', () {
      test('User pauses, closes modal, reopens, resumes - timer continues',
          () async {
