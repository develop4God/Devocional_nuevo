...
1. The app lifecycle changed (background → foreground)
...
void didPopNext() {
  ...
  _streakFuture = _loadStreak();
  debugPrint('📄 DevocionalesPage popped next → tracking resumed & streak refreshed');
}
```
...
- `Icons.grid_view` → `Icons.grid_view_rounded`
- `Icons.view_carousel` → `Icons.view_carousel_rounded`
...
- `layout`: `SwiperLayout.STACK` → `SwiperLayout.DEFAULT`
- `viewportFraction`: `0.88` → `0.85`
- `scale`: `0.92` → `0.9`
- `curve`: `Curves.easeInOutCubic` → `Curves.easeOutQuart`
- `duration`: `350ms` → `280ms`
...
✅ **Before**: Stuck, unresponsive, requiring multiple swipes
✅ **After**: Smooth, fluid, single-swipe navigation
...
- `lib/pages/discovery_list_page.dart` - Floating grid toggle button + Swiper configuration
...
**Bug Count:** 3 bugs fixed (Streak, Grid Button, Swiper Gestures)
