...
void main() {
  group('Devotional Reading Logic Tests', () {
    ...
    test('DevocionalProvider recordDevocionalRead works correctly', () async {
      ...
      // Record a devotional read with sufficient criteria (60s reading, 80% scroll)
    ...
    test('Rapid tapping prevention works', () async {
      ...
      // Record a devotional with sufficient criteria
      await statsService.recordDevocionalRead(
    ...
    test('Legitimate re-reading after time delay is not prevented', () async {
      ...
      // Record initial read with sufficient criteria
      await statsService.recordDevocionalRead(
    ...
    test('Favorites count integration with devotional reading', () async {
      ...
      // Record devotional read with favorites count and sufficient criteria
      await statsService.recordDevocionalRead(
    ...
    test('Achievement unlocking during devotional reading', () async {
      ...
      // Record first devotional to unlock "Primer Paso" achievement with sufficient criteria
      await statsService.recordDevocionalRead(
    ...
    test('Streak calculation across multiple days simulation', () async {
      ...
      // Record devotional reads with sufficient criteria (in real scenario, these would be on different days)
      await statsService.recordDevocionalRead(
      ...
      // Record another devotional (same day, so streak should remain 1) with sufficient criteria
      await statsService.recordDevocionalRead(
    ...
    test('Service handles malformed data gracefully', () async {
      ...
      // Try to record with null-like values but sufficient criteria
      try {
        await statsService.recordDevocionalRead(
          devocionalId: '',
          readingTimeSeconds: 60,
    ...
    test('Multiple achievements unlock correctly', () async {
      ...
      // Record multiple devotionals to unlock reading-based achievements with sufficient criteria
      for (int i = 1; i <= 7; i++) {
        await statsService.recordDevocionalRead(
