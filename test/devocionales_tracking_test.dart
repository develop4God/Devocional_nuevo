...
void main() {
  group('Devocionales Tracking Integration Tests', () {
    ...
    test('Stats service correctly records devotionals and triggers review logic', () async {
      ...
      var shouldShow = await InAppReviewService.shouldShowReviewRequest(stats.totalDevocionalesRead);
      expect(shouldShow, isFalse, reason: '4 devotionals should not trigger review');
      ...
      shouldShow = await InAppReviewService.shouldShowReviewRequest(stats.totalDevocionalesRead);
    ...
    test('Audio completion also triggers review logic', () async {
      ...
      final shouldShow = await InAppReviewService.shouldShowReviewRequest(stats.totalDevocionalesRead);
      expect(shouldShow, isTrue, reason: 'Mixed reading/audio to 5 should trigger review');
    });
    ...
    test('All milestone values work correctly', () async {
      ...
      for (final milestone in milestones) {
        ...
        expect(shouldShow, isTrue, reason: 'Milestone $milestone should trigger review');
      }
    });
    ...
    test('Review state persistence works correctly', () async {
      ...
      expect(shouldShow, isFalse, reason: 'Should not show if user already rated');
      ...
      expect(shouldShow, isFalse, reason: 'Should not show if user chose never ask again');
      
      // Clear all restrictions
      await InAppReviewService.clearAllPreferences();
      
      shouldShow = await InAppReviewService.shouldShowReviewRequest(50);
      expect(shouldShow, isTrue, reason: 'Should show after clearing restrictions');
    });
    ...
    test('Sequential recording maintains correct count', () async {
      ...
      for (int i = 1; i <= 7; i++) {
        ...
        final stats = await statsService.getStats();
        expect(stats.totalDevocionalesRead, equals(i), reason: 'Count should be $i after $i recordings');
        
        // Check milestone detection
        final shouldShow = await InAppReviewService.shouldShowReviewRequest(stats.totalDevocionalesRead);
        if (i == 5) {
          expect(shouldShow, isTrue, reason: 'Should trigger review at 5th devotional');
        } else {
          expect(shouldShow, isFalse, reason: 'Should not trigger review at $i devotionals');
        }
    ...
    test('Review request frequency limits work', () async {
      ...
      shouldShow = await InAppReviewService.shouldShowReviewRequest(25);
      expect(shouldShow, isFalse, reason: 'Should not show next milestone during cooldown');
    });
