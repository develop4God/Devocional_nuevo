...
void main() {
  ...
  group('Issue #46 - Multilingual Ordinals Integration Test', () {
    ...
    test(
        'Issue #46: Spanish functionality remains unchanged (backward compatibility)',
        () {
      // Ensure production Spanish users see no changes
      ttsService.setLanguageContext('es', 'RVR1960');

      expect(ttsService.formatBibleBook('1 Juan'), equals('Primera de Juan'));
      expect(ttsService.formatBibleBook('2 Timoteo'),
          equals('Segunda de Timoteo'));
      expect(ttsService.formatBibleBook('3 Juan'), equals('Tercera de Juan'));
      expect(ttsService.formatBibleBook('Génesis'),
          equals('Génesis')); // No ordinal
      expect(ttsService.formatBibleBook('Apocalipsis'),
          equals('Apocalipsis')); // No ordinal
    });
  });
}
