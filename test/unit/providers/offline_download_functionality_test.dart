...
void main() {
  ...
  group('Offline Download Logic Tests', () {
    ...
    test(
        'should generate correct API URLs for all language/version combinations',
        () {
      const testYear = 2025;

      // Test Spanish backward compatibility
      final spanishRvr1960Url = Constants.getDevocionalesApiUrlMultilingual(
          testYear, 'es', 'RVR1960');
      expect(
          spanishRvr1960Url,
          equals(
      ...
      // Test Portuguese NVI
      final portugueseNviUrl =
          Constants.getDevocionalesApiUrlMultilingual(testYear, 'pt', 'NVI');
      expect(
          portugueseNviUrl,
          equals(
              'https://raw.githubusercontent.com/develop4God/Devocionales-json/refs/heads/main/Devocional_year_${testYear}_pt_NVI.json'));
      ...
      expect(
          frenchLsgUrl,
          equals(
              'https://raw.githubusercontent.com/develop4God/Devocionales-json/refs/heads/main/Devocional_year_${testYear}_fr_LSG1910.json'));

      // Test French TOB
      final frenchTobUrl =
          Constants.getDevocionalesApiUrlMultilingual(testYear, 'fr', 'TOB');
      expect(
          frenchTobUrl,
          equals(
              'https://raw.githubusercontent.com/develop4God/Devocionales-json/refs/heads/main/Devocional_year_${testYear}_fr_TOB.json'));
    });
    ...
    test('should maintain backward compatibility for Spanish RVR1960', () {
      ...
      final multilingualUrl = Constants.getDevocionalesApiUrlMultilingual(
          testYear, 'es', 'RVR1960');
    ...
    test('should support multilingual offline functionality', () async {
      ...
      for (final config in testConfigs) {
        ...
        final url = Constants.getDevocionalesApiUrlMultilingual(
            DateTime.now().year, config['lang']!, config['version']!);
        expect(url, isNotEmpty);
        expect(url, startsWith('https://'));
    ...
    test('should support all required languages and versions', () {
      ...
      expect(Constants.bibleVersionsByLanguage.keys,
          containsAll(['es', 'en', 'pt', 'fr']));
      expect(Constants.defaultVersionByLanguage.keys,
          containsAll(['es', 'en', 'pt', 'fr']));
