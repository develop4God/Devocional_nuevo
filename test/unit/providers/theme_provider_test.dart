  ...
  group('Theme Riverpod Provider Tests', () {
    ...
    group('Initial Theme Loading', () {
      test('should load default theme when no saved preferences', () async {
        ...
        expect(loadedState.themeData, equals(appThemeFamilies['Deep Purple']!['light']!));
      });
      ...
      test('should fallback to default when invalid theme family is saved', () async {
        // Arrange - repository returns invalid theme but handles it internally
        when(() => mockRepo.getThemePreferences()).thenAnswer((_) async => (
          themeFamily: 'Deep Purple', // Repository should handle invalid themes internally
          brightness: 'light'
        ));
    ...
    group('Theme Family Changes', () {
      test('should update theme family and persist to storage', () async {
        ...
        expect(state.brightness, equals(Brightness.light)); // Should preserve brightness
      ...
      test('should ignore invalid theme family changes', () async {
        ...
        await container.read(themeProvider.notifier).setThemeFamily('InvalidTheme');
    ...
    group('Brightness Changes', () {
      test('should update brightness and persist to storage', () async {
        ...
        await container.read(themeProvider.notifier).setBrightness(Brightness.dark);

        // Assert - state should be updated
        final state = container.read(themeProvider) as ThemeStateLoaded;
        expect(state.themeFamily, equals('Light Blue')); // Should preserve theme family
        expect(state.brightness, equals(Brightness.dark));
        expect(state.themeData, equals(appThemeFamilies['Light Blue']!['dark']!));
    ...
    group('Convenience Providers', () {
      ...
      test('currentBrightnessProvider should return correct value', () async {
        ...
        // Assert
        expect(container.read(currentBrightnessProvider), equals(Brightness.dark));
      });

      test('currentThemeDataProvider should return correct ThemeData', () async {
        // Arrange
        when(() => mockRepo.getThemePreferences()).thenAnswer((_) async => (
          themeFamily: 'Pink',
          brightness: 'light'
      ...
      test('dividerAdaptiveColorProvider should return correct color', () async {
        // Test light mode
        when(() => mockRepo.getThemePreferences()).thenAnswer((_) async => (
          themeFamily: 'Green',
          brightness: 'light'
        ...
        await pumpEventQueue();
        expect(container.read(dividerAdaptiveColorProvider), equals(Colors.black));
        ...
        await pumpEventQueue();
        expect(container.read(dividerAdaptiveColorProvider), equals(Colors.white));
      });

      test('themeLoadingProvider should indicate loading state correctly', () async {
        // Initially loading
    ...
    group('Error Handling', () {
      test('should handle repository exceptions gracefully', () async {
        ...
        when(() => mockRepo.getThemePreferences()).thenThrow(Exception('Storage error'));
      ...
      test('should handle setThemeFamily storage failures gracefully', () async {
        // Arrange
        when(() => mockRepo.getThemePreferences()).thenAnswer((_) async => (
          themeFamily: 'Green',
          brightness: 'light'
        ));
        when(() => mockRepo.setThemeFamily(any())).thenThrow(Exception('Write failed'));
    ...
    group('Behavioral Integration Tests', () {
      test('theme changes should be immediately reflected in all providers', () async {
        // Arrange
        when(() => mockRepo.getThemePreferences()).thenAnswer((_) async => (
          themeFamily: 'Deep Purple',
          brightness: 'light'
      ...
      test('theme persistence should survive app restarts (simulated)', () async {
