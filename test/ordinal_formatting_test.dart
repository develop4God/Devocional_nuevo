...
void main() {
  group('Bible Text Formatter Tests', () {
    ...
    test('should handle edge cases and invalid input gracefully', () {
      ...
      expect(
          BibleTextFormatter.formatBibleBook('4 Juan', 'es'), equals('4 Juan'));

      // Non-existing books (should still apply ordinal format)
      expect(BibleTextFormatter.formatBibleBook('1 Invented', 'es'),
          contains('Primera de'));
      ...
      expect(
          ...
          equals(
              '  1 Juan  ')); // Leading/trailing whitespace should be preserved if pattern doesn't match at start
    });

    test('should default to Spanish for unknown languages', () {
      expect(BibleTextFormatter.formatBibleBook('1 Juan', 'xx'),
          contains('Primera de Juan'));
    });
  });
}
